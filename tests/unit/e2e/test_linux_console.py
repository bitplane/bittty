...
def test_linux_set_palette_entry():
    ...
    assert terminal.palette.colors[4] == (0xA0, 0xB0, 0xC0)
    assert terminal.blitter.current_buffer.get_line_text(0).startswith("done")
...
def test_linux_set_palette_split_across_feeds():
    ...
    for ch in "\x1b]P1ffffff":
        parser.feed(ch)
    parser.feed("X")
    assert terminal.palette.colors[1] == (255, 255, 255)
    assert terminal.blitter.current_buffer.get_line_text(0).startswith("X")


def test_linux_reset_palette():
    terminal, parser = _term(model=LINUX)
    parser.feed("\x1b]P200ff00")  # recolour entry 2
    assert terminal.palette.colors[2] == (0x00, 0xFF, 0x00)
    parser.feed("\x1b]R")  # reset the whole palette
    assert terminal.palette.colors[2] == (0, 170, 0)  # VGA green restored
...
def test_linux_recognises_the_ibm_pc_charset_designator():
    ...
    assert terminal.charset.g0_charset == "U"
