...
def test_alternate_buffer_enable(parser, terminal):
    ...
    primary_text = terminal.board.screen.current_buffer.get_line_text(0).strip()
    ...
    alt_text = terminal.board.screen.current_buffer.get_line_text(0).strip()
...
def test_alternate_buffer_persistence(parser, terminal):
    ...
    line0 = terminal.board.screen.current_buffer.get_line_text(0).strip()
    line1 = terminal.board.screen.current_buffer.get_line_text(1).strip()
    ...
    alt_line1 = terminal.board.screen.current_buffer.get_line_text(1).strip()
    alt_line2 = terminal.board.screen.current_buffer.get_line_text(2).strip()
...
def test_alternate_buffer_mixed_operations(parser, terminal):
    ...
    assert terminal.board.screen.in_alt_screen is True  # Still in alt screen

    # Disable alternate buffer
    parser.feed("\x1b[?1049l")
    assert terminal.board.screen.in_alt_screen is False  # Back to primary
