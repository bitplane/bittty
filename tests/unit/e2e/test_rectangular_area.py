...
def _rows(terminal):
    ...
    return [buf.get_line_text(y) for y in range(terminal.height)]
...
def test_decera_erases_a_rectangle():
    ...
    assert _rows(terminal)[2] == "XX    XXXX"  # middle punched out
...
def test_deccra_copies_a_rectangle():
    ...
    assert _rows(terminal)[2].startswith("ABCD")
...
def test_decsera_rectangle_respects_protection():
    ...
    assert _rows(terminal)[0].startswith("AB  ")  # AB protected, CD erased
