...
def test_keypad_application_mode():
    ...
    assert not terminal.modes.application_keypad
...
def test_keypad_mode_sequences_with_text():
    ...
    assert not terminal.modes.application_keypad  # Should end in normal mode
...
def test_csi_cursor_save_restore():
    ...
    assert terminal.cursor.x == 4  # 0-based
    assert terminal.cursor.y == 4  # 0-based

    # Restore cursor
    parser.feed("\x1b[u")  # Restore cursor
    assert terminal.cursor.x == 19  # 0-based (was col 20)
    assert terminal.cursor.y == 9  # 0-based (was row 10)
