...
def test_spa_epa_toggle_protection():
    ...
    assert terminal.board.style.current.protected is True
    parser.feed("\x1bW")  # EPA (ESC W) — end protected area
    assert terminal.board.style.current.protected is None
...
def test_c1_transmission_select():
    ...
    assert terminal.board.c1_eightbit is True
    parser.feed("\x1b F")  # ESC SP F — S7C1T
    assert terminal.board.c1_eightbit is False


def test_ansi_conformance_level():
    terminal, parser = _term()
    parser.feed("\x1b M")  # ESC SP M — ANSI conformance level 2
    assert terminal.board.ansi_conformance_level == 2
    parser.feed("\x1b L")  # ESC SP L — level 1
    assert terminal.board.ansi_conformance_level == 1
...
def test_mode_tail_reports_via_decrqm():
    ...
    rec = Rec()
    terminal.board.host.attach(rec)
