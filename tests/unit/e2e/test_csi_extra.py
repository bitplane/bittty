...
def test_cnl_and_cpl_move_to_column_zero():
    ...
    assert (terminal.cursor.x, terminal.cursor.y) == (0, 6)
    parser.feed("\x1b[3;10H\x1b[1F")  # CPL 1: up 1 line, column 0
    assert (terminal.cursor.x, terminal.cursor.y) == (0, 1)
...
def test_decscusr_sets_cursor_shape_and_blink():
    ...
    assert terminal.cursor.shape == "underline"
    assert terminal.modes.cursor_blinking is False
    parser.feed("\x1b[5 q")  # blinking bar
    assert terminal.cursor.shape == "bar"
    assert terminal.modes.cursor_blinking is True
