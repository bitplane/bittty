...
def test_csi_cup_cursor_position(standard_terminal: Board):
    ...
    assert standard_terminal.board.cursor.x == 19  # 0-based
    assert standard_terminal.board.cursor.y == 9  # 0-based
...
def test_csi_cuu_cursor_up(standard_terminal: Board):
    ...
    parser = Parser(standard_terminal.board)
    standard_terminal.board.cursor.y = 10
    parser.feed(f"{ESC}[5A")  # ESC[5A -> move up 5 rows
    assert standard_terminal.board.cursor.y == 5
...
def test_csi_cuu_cursor_up_no_param(standard_terminal: Board):
    ...
    assert standard_terminal.board.cursor.y == 9
...
def test_csi_cub_cursor_backward(standard_terminal: Board):
    ...
    assert standard_terminal.board.cursor.x == 5
...
def test_csi_cha_cursor_horizontal_absolute(standard_terminal: Board):
    ...
    assert standard_terminal.board.cursor.x == 9  # 0-based
...
def test_csi_vpa_vertical_position_absolute(standard_terminal: Board):
    ...
    assert standard_terminal.board.cursor.y == 4  # 0-based
...
def test_csi_ech_erase_character_preserves_background_color(standard_terminal: Board):
    ...
    standard_terminal.board.cursor.x = 1  # Position at 'E'
...
def test_csi_decsc_and_decrc_save_restore_cursor(standard_terminal: Board):
    ...
    parser = Parser(standard_terminal.board)
    standard_terminal.board.cursor.x = 15
    standard_terminal.board.cursor.y = 10
    parser.feed("\x1b[s")  # Save cursor
    standard_terminal.board.cursor.x = 0
    standard_terminal.board.cursor.y = 0
    parser.feed("\x1b[u")  # Restore cursor
    assert standard_terminal.board.cursor.x == 15
    assert standard_terminal.board.cursor.y == 10
