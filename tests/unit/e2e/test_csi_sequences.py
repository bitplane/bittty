...
def test_csi_cup_cursor_position(standard_terminal: Board):
    ...
    assert standard_terminal.cursor.x == 19  # 0-based
    assert standard_terminal.cursor.y == 9  # 0-based


def test_csi_cup_cursor_position_no_params(standard_terminal: Board):
    """Test CSI H (CUP) with no parameters (defaults to 1;1)."""
    parser = Parser(standard_terminal)
    parser.feed(f"{ESC}[H")  # ESC[H -> move to row 1, col 1
    assert standard_terminal.cursor.x == 0
    assert standard_terminal.cursor.y == 0


def test_csi_cuu_cursor_up(standard_terminal: Board):
    """Test CSI A (CUU - Cursor Up) with parameter."""
    parser = Parser(standard_terminal)
    standard_terminal.cursor.y = 10
    parser.feed(f"{ESC}[5A")  # ESC[5A -> move up 5 rows
    assert standard_terminal.cursor.y == 5


def test_csi_cuu_cursor_up_no_param(standard_terminal: Board):
    """Test CSI A (CUU) with no parameter (defaults to 1)."""
    parser = Parser(standard_terminal)
    standard_terminal.cursor.y = 10
    parser.feed(f"{ESC}[A")  # ESC[A -> move up 1 row
    assert standard_terminal.cursor.y == 9


def test_csi_cud_cursor_down(standard_terminal: Board):
    """Test CSI B (CUD - Cursor Down) with parameter."""
    parser = Parser(standard_terminal)
    standard_terminal.cursor.y = 10
    parser.feed(f"{ESC}[5B")  # ESC[5B -> move down 5 rows
    assert standard_terminal.cursor.y == 15


def test_csi_cuf_cursor_forward(standard_terminal: Board):
    """Test CSI C (CUF - Cursor Forward) with parameter."""
    parser = Parser(standard_terminal)
    standard_terminal.cursor.x = 10
    parser.feed("\x1b[5C")  # ESC[5C -> move forward 5 columns
    assert standard_terminal.cursor.x == 15


def test_csi_cub_cursor_backward(standard_terminal: Board):
    """Test CSI D (CUB - Cursor Backward) with parameter."""
    parser = Parser(standard_terminal)
    standard_terminal.cursor.x = 10
    parser.feed("\x1b[5D")  # ESC[5D -> move backward 5 columns
    assert standard_terminal.cursor.x == 5


def test_csi_ed_erase_in_display(standard_terminal: Board):
    """Test CSI J (ED - Erase in Display) with parameter."""
    parser = Parser(standard_terminal)
    standard_terminal.blitter.write_text("text")
    ...
    assert standard_terminal.cursor.x == 4
    assert standard_terminal.cursor.y == 0
...
def test_csi_el_erase_in_line(standard_terminal: Board):
    ...
    parser = Parser(standard_terminal)
    standard_terminal.blitter.write_text("some text")
    standard_terminal.cursor.x = 4
    parser.feed("\x1b[0K")  # ESC[0K -> clear from cursor to end of line
    line = "".join(char for _, char in standard_terminal.blitter.current_buffer.get_content()[0])
...
def test_csi_ich_insert_characters(standard_terminal: Board):
    ...
    parser = Parser(standard_terminal)
    standard_terminal.blitter.write_text("abcdef")
    standard_terminal.cursor.x = 2
...
def test_csi_il_insert_lines(standard_terminal: Board):
    ...
    parser = Parser(standard_terminal)
    standard_terminal.blitter.write_text("line 1")
    standard_terminal.cursor.line_feed()
    standard_terminal.blitter.write_text("line 2")
    standard_terminal.cursor.y = 0
...
def test_csi_dl_delete_lines(standard_terminal: Board):
    ...
    standard_terminal.blitter.write_text("line 2")
    standard_terminal.cursor.line_feed()
    standard_terminal.blitter.write_text("line 3")
    standard_terminal.cursor.y = 0
...
def test_csi_decstbm_set_scroll_region(standard_terminal: Board):
    ...
    assert standard_terminal.blitter.scroll_top == 4
    assert standard_terminal.blitter.scroll_bottom == 14
...
def test_csi_cha_cursor_horizontal_absolute(standard_terminal: Board):
    ...
    assert standard_terminal.cursor.x == 9  # 0-based


def test_csi_vpa_vertical_position_absolute(standard_terminal: Board):
    """Test CSI d (VPA - Vertical Position Absolute) with parameter."""
    parser = Parser(standard_terminal)
    parser.feed("\x1b[5d")  # ESC[5d -> move cursor to row 5
    assert standard_terminal.cursor.y == 4  # 0-based


def test_csi_ech_erase_character(standard_terminal: Board):
    """Test CSI X (ECH - Erase Character)."""
    parser = Parser(standard_terminal)
    standard_terminal.blitter.write_text("before-text-after")
    standard_terminal.cursor.x = 7
    parser.feed("\x1b[4X")  # Erase 4 characters
    line = "".join(char for _, char in standard_terminal.blitter.current_buffer.get_content()[0])
    assert "before-    -after" in line


def test_csi_ech_erase_character_preserves_background_color(standard_terminal: Board):
    ...
    standard_terminal.cursor.x = 1  # Position at 'E'
    parser.feed("\x1b[3X")  # Erase 3 characters (ELL)

    # Check that erased characters have green background
    row = standard_terminal.blitter.current_buffer.get_content()[0]
...
def test_csi_decsc_and_decrc_save_restore_cursor(standard_terminal: Board):
    """Test CSI s (DECSC) and CSI u (DECRC) for saving and restoring cursor."""
    parser = Parser(standard_terminal)
    standard_terminal.cursor.x = 15
    standard_terminal.cursor.y = 10
    parser.feed("\x1b[s")  # Save cursor
    standard_terminal.cursor.x = 0
    standard_terminal.cursor.y = 0
    parser.feed("\x1b[u")  # Restore cursor
    assert standard_terminal.cursor.x == 15
    assert standard_terminal.cursor.y == 10


def test_csi_truly_unhandled_sequences(standard_terminal: Board):
    ...
    for seq in unhandled_sequences:
        standard_terminal.blitter.write_text("before")
        parser.feed(seq)
        standard_terminal.blitter.write_text("after")
        line = "".join(char for _, char in standard_terminal.blitter.current_buffer.get_content()[0])
        assert "beforeafter" in line.strip()
        standard_terminal.blitter.clear_screen(2)  # Clear screen for next test


def test_csi_ech_does_not_move_the_cursor(standard_terminal: Board):
    """ECH erases in place; the cursor must not advance."""
    parser = Parser(standard_terminal)
    standard_terminal.blitter.write_text("abcdefgh")
    standard_terminal.cursor.x = 2
    parser.feed("\x1b[3X")
    assert standard_terminal.blitter.current_buffer.get_line_text(0).startswith("ab   fgh")
    assert standard_terminal.cursor.x == 2
