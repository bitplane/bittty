...
def test_csi_cup_cursor_position(standard_board: Board):
    """Test CSI H (CUP - Cursor Position) with parameters."""
    parser = Parser(standard_board)
    parser.feed(f"{ESC}[10;20H")  # ESC[10;20H -> move to row 10, col 20
    assert standard_board.cursor.x == 19  # 0-based
    assert standard_board.cursor.y == 9  # 0-based


def test_csi_cup_cursor_position_no_params(standard_board: Board):
    """Test CSI H (CUP) with no parameters (defaults to 1;1)."""
    parser = Parser(standard_board)
    parser.feed(f"{ESC}[H")  # ESC[H -> move to row 1, col 1
    assert standard_board.cursor.x == 0
    assert standard_board.cursor.y == 0


def test_csi_cuu_cursor_up(standard_board: Board):
    """Test CSI A (CUU - Cursor Up) with parameter."""
    parser = Parser(standard_board)
    standard_board.cursor.y = 10
    parser.feed(f"{ESC}[5A")  # ESC[5A -> move up 5 rows
    assert standard_board.cursor.y == 5


def test_csi_cuu_cursor_up_no_param(standard_board: Board):
    """Test CSI A (CUU) with no parameter (defaults to 1)."""
    parser = Parser(standard_board)
    standard_board.cursor.y = 10
    parser.feed(f"{ESC}[A")  # ESC[A -> move up 1 row
    assert standard_board.cursor.y == 9


def test_csi_cud_cursor_down(standard_board: Board):
    """Test CSI B (CUD - Cursor Down) with parameter."""
    parser = Parser(standard_board)
    standard_board.cursor.y = 10
    parser.feed(f"{ESC}[5B")  # ESC[5B -> move down 5 rows
    assert standard_board.cursor.y == 15


def test_csi_cuf_cursor_forward(standard_board: Board):
    """Test CSI C (CUF - Cursor Forward) with parameter."""
    parser = Parser(standard_board)
    standard_board.cursor.x = 10
    parser.feed("\x1b[5C")  # ESC[5C -> move forward 5 columns
    assert standard_board.cursor.x == 15


def test_csi_cub_cursor_backward(standard_board: Board):
    """Test CSI D (CUB - Cursor Backward) with parameter."""
    parser = Parser(standard_board)
    standard_board.cursor.x = 10
    parser.feed("\x1b[5D")  # ESC[5D -> move backward 5 columns
    assert standard_board.cursor.x == 5


def test_csi_ed_erase_in_display(standard_board: Board):
    """Test CSI J (ED - Erase in Display) with parameter."""
    parser = Parser(standard_board)
    standard_board.blitter.write_text("text")
    parser.feed("\x1b[2J")  # ESC[2J -> clear entire screen
    content = "".join("".join(char for _, char in line) for line in standard_board.get_content())
    assert content.strip() == ""
    # ED 2 erases without moving the cursor (xterm behaviour).
    assert standard_board.cursor.x == 4
    assert standard_board.cursor.y == 0


def test_csi_el_erase_in_line(standard_board: Board):
...
def test_csi_ich_insert_characters(standard_board: Board):
...
def test_csi_dch_delete_characters(standard_board: Board):
...
def test_csi_il_insert_lines(standard_board: Board):
    """Test CSI L (IL - Insert Lines) with parameter."""
    parser = Parser(standard_board)
    standard_board.blitter.write_text("line 1")
    standard_board.cursor.line_feed()
    standard_board.blitter.write_text("line 2")
    standard_board.cursor.y = 0
    parser.feed("\x1b[2L")  # ESC[2L -> insert 2 lines
    content = standard_board.get_content()
...
def test_csi_dl_delete_lines(standard_board: Board):
    ...
    standard_board.blitter.write_text("line 2")
    standard_board.cursor.line_feed()
    standard_board.blitter.write_text("line 3")
    standard_board.cursor.y = 0
...
def test_csi_su_scroll_up(standard_board: Board):
...
def test_csi_sd_scroll_down(standard_board: Board):
...
def test_csi_decstbm_set_scroll_region(standard_board: Board):
    ...
    assert standard_board.blitter.scroll_top == 4
    assert standard_board.blitter.scroll_bottom == 14


def test_csi_cha_cursor_horizontal_absolute(standard_board: Board):
    """Test CSI G (CHA - Cursor Horizontal Absolute) with parameter."""
    parser = Parser(standard_board)
    parser.feed("\x1b[10G")  # ESC[10G -> move cursor to column 10
    assert standard_board.cursor.x == 9  # 0-based


def test_csi_vpa_vertical_position_absolute(standard_board: Board):
    """Test CSI d (VPA - Vertical Position Absolute) with parameter."""
    parser = Parser(standard_board)
    parser.feed("\x1b[5d")  # ESC[5d -> move cursor to row 5
    assert standard_board.cursor.y == 4  # 0-based


def test_csi_ech_erase_character(standard_board: Board):
...
def test_csi_ech_erase_character_preserves_background_color(standard_board: Board):
    ...
    standard_board.cursor.x = 1  # Position at 'E'
...
def test_csi_decsc_and_decrc_save_restore_cursor(standard_board: Board):
    """Test CSI s (DECSC) and CSI u (DECRC) for saving and restoring cursor."""
    parser = Parser(standard_board)
    standard_board.cursor.x = 15
    standard_board.cursor.y = 10
    parser.feed("\x1b[s")  # Save cursor
    standard_board.cursor.x = 0
    standard_board.cursor.y = 0
    parser.feed("\x1b[u")  # Restore cursor
    assert standard_board.cursor.x == 15
    assert standard_board.cursor.y == 10


def test_csi_truly_unhandled_sequences(standard_board: Board):
...
def test_csi_ech_does_not_move_the_cursor(standard_board: Board):
