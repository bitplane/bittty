...
def test_a_callable_sink_is_supported():
    terminal, parser = _term()
    captured = []
    terminal.printer.attach(captured.append)
    parser.feed("ok")
    parser.feed("\x1b[0i")
    assert captured == ["ok\n\n\n"]
...
def test_ris_clears_printer_modes_but_keeps_the_sink():
    ...
    assert terminal.printer.controller_mode is False
    assert terminal.printer.sink is paper  # config survives a reset
