...
def test_decdwl_sets_double_width_on_the_cursor_line():
    ...
    assert _attr(board, 2) == constants.LINE_DOUBLE_WIDTH
    assert _attr(board, 0) == constants.LINE_SINGLE
...
def test_decdhl_top_and_bottom():
    ...
    assert _attr(board, 0) == constants.LINE_DOUBLE_TOP
    assert _attr(board, 1) == constants.LINE_DOUBLE_BOTTOM
...
def test_line_attribute_travels_with_the_line_on_scroll_up():
    ...
    assert _attr(board, 1) == constants.LINE_DOUBLE_WIDTH
    assert _attr(board, 2) == constants.LINE_SINGLE
...
def test_line_attribute_travels_with_the_line_on_insert_line():
    ...
    assert _attr(board, 0) == constants.LINE_SINGLE  # the fresh blank line
    assert _attr(board, 1) == constants.LINE_DOUBLE_WIDTH  # the double line moved down
...
def test_resize_keeps_attributes_aligned_with_rows():
    ...
    board.resize(10, 6)  # grow
    assert _attr(board, 1) == constants.LINE_DOUBLE_WIDTH
    assert _attr(board, 5) == constants.LINE_SINGLE
