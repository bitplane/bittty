...
def test_origin_mode_makes_cup_relative_to_the_scroll_region():
    ...
    assert (terminal.cursor.x, terminal.cursor.y) == (0, 2)  # top of region
...
def test_decaln_fills_the_screen_and_does_not_leak_an_eight():
    terminal, parser = _term()
    parser.feed("\x1b#8")
    assert terminal.blitter.current_buffer.get_line_text(0) == "E" * 20  # filled, not a stray "8"
