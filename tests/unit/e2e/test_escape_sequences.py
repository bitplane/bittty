...
def test_bell_character(terminal):
    ...
    initial_cursor = (terminal.cursor.x, terminal.cursor.y)
    initial_text = terminal.blitter.current_buffer.get_line_text(0)

    parser.feed(BEL)

    # BEL should not cause visible terminal changes
    assert (terminal.cursor.x, terminal.cursor.y) == initial_cursor
    assert terminal.blitter.current_buffer.get_line_text(0) == initial_text
...
def test_ris_reset_terminal(terminal):
    ...
    initial_content = terminal.blitter.current_buffer.get_line_text(0)
    assert "Hello World" in initial_content
    assert terminal.cursor.y == 4  # 0-indexed
    assert terminal.cursor.x == 9  # 0-indexed
    ...
    cleared_content = terminal.blitter.current_buffer.get_line_text(0)
...
def test_ri_reverse_index_no_scroll(terminal):
    ...
    assert terminal.cursor.y == 5  # 0-indexed

    # Send reverse index
    parser.feed("\x1bM")  # ESC then M (RI)

    # Should move cursor up one line
    assert terminal.cursor.y == 4
...
def test_desc_save_cursor(terminal):
    ...
    assert terminal.cursor.y == 9  # 0-indexed
    assert terminal.cursor.x == 19  # 0-indexed
...
def test_decrc_restore_cursor(terminal):
    ...
    assert terminal.cursor.y != 4
    assert terminal.cursor.x != 14

    # Restore cursor
    parser.feed("\x1b8")  # ESC then 8 (DECRC)

    # Should be back at saved position
    assert terminal.cursor.y == 4  # 0-indexed from line 5
    assert terminal.cursor.x == 14  # 0-indexed from column 15
