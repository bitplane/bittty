...
def test_rep_basic(small_terminal):
    ...
    line = small_terminal.board.screen.current_buffer.get_line_text(0)
...
def test_rep_with_different_counts():
    ...
    assert terminal.board.screen.current_buffer.get_line_text(0)[:2] == "XX"

    # Test count = 10
    parser.feed("=")
    parser.feed(f"{ESC}[10b")
    assert terminal.board.screen.current_buffer.get_line_text(0)[2:13] == "==========="
...
def test_rep_with_no_parameter():
    ...
    assert terminal.board.screen.current_buffer.get_line_text(0)[:2] == "ZZ"
...
def test_rep_with_no_previous_character():
    ...
    assert terminal.board.screen.current_buffer.get_line_text(0)[:5] == "     "
