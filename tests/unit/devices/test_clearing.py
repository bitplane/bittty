...
def test_clear_rect():
    ...
    for y in range(5):
        for x in range(10):
            board.blitter.current_page.set_cell(x, y, "X")
    ...
    for y in range(5):
        for x in range(10):
            if 1 <= y <= 3 and 2 <= x <= 5:
                ...
                assert board.blitter.current_page.get_cell(x, y) == (Style(), "X")
...
def test_clear_rect_with_style():
    ...
    for y in range(5):
        for x in range(10):
            board.blitter.current_page.set_cell(x, y, "X", f"\x1b[{31 + y}m")
    ...
    for y in range(5):
        for x in range(10):
            if 1 <= y <= 3 and 2 <= x <= 5:
                assert board.blitter.current_page.get_cell(x, y) == (yellow_style, " ")
            else:
                expected_style = parse_sgr_sequence(f"\x1b[{31 + y}m")
                assert board.blitter.current_page.get_cell(x, y) == (expected_style, "X")
...
def test_clear_line_from_cursor_to_end():
    ...
    assert board.blitter.current_page.get_line_text(0) == "01234     "
...
def test_clear_line_from_beginning_to_cursor():
    ...
    assert board.blitter.current_page.get_line_text(0) == "      6789"
