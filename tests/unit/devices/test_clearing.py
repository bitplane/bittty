...
def test_clear_rect():
    ...
    terminal.board.screen.clear_rect(2, 1, 5, 3)
    ...
    for y in range(5):
        for x in range(10):
            if 1 <= y <= 3 and 2 <= x <= 5:
                ...
                assert terminal.board.screen.current_buffer.get_cell(x, y) == (Style(), "X")
...
def test_clear_line_from_cursor_to_end():
    ...
    assert terminal.board.screen.current_buffer.get_line_text(0) == "01234     "
...
def test_clear_line_from_beginning_to_cursor():
    ...
    terminal.board.cursor.x = 5
    terminal.board.cursor.y = 0
    terminal.board.screen.clear_line(ERASE_FROM_START_TO_CURSOR)
    assert terminal.board.screen.current_buffer.get_line_text(0) == "      6789"
...
def test_clear_line_with_mixed_styles():
    ...
    terminal.board.cursor.y = 0
    terminal.board.screen.clear_line(0)  # Clear from cursor to end
    assert terminal.board.screen.current_buffer.get_line_text(0) == "ABC       "

    terminal.board.screen.current_buffer.set(0, 1, "ABCDEFGHI")
    terminal.board.cursor.x = 3
    terminal.board.cursor.y = 1
    terminal.board.screen.clear_line(1)  # Clear from beginning to cursor
    assert terminal.board.screen.current_buffer.get_line_text(1) == "    EFGHI "
...
def test_clear_screen_invalid_cursor():
    ...
    assert all(c == " " for c in terminal.board.screen.current_buffer.get_line_text(4))
