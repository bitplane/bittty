...
def test_write_cell_overwrite():
    ...
    assert board.blitter.current_buffer.get_cell(0, 0) == (parse_sgr_sequence("\x1b[31m"), "A")
    assert board.cursor.x == 1

    parser.feed("\x1b[32m")  # Set green color
    parser.feed("B")
    assert board.blitter.current_buffer.get_line_text(0) == "AB        "
    assert board.blitter.current_buffer.get_cell(1, 0) == (parse_sgr_sequence("\x1b[32m"), "B")
    assert board.cursor.x == 2

    board.cursor.set_position(0, 0)
    parser.feed("\x1b[34m")  # Set blue color
    parser.feed("C")
    assert board.blitter.current_buffer.get_line_text(0) == "CB        "
    assert board.blitter.current_buffer.get_cell(0, 0) == (parse_sgr_sequence("\x1b[34m"), "C")
    assert board.cursor.x == 1
...
def test_write_cell_insert_mode():
    ...
    parser.feed("\x1b[34m")  # Set blue color
    parser.feed("C")
    assert board.blitter.current_buffer.get_line_text(0) == "CAB       "
    assert board.blitter.current_buffer.get_cell(0, 0) == (parse_sgr_sequence("\x1b[34m"), "C")
    assert board.blitter.current_buffer.get_cell(1, 0) == (parse_sgr_sequence("\x1b[31m"), "A")
    assert board.blitter.current_buffer.get_cell(2, 0) == (parse_sgr_sequence("\x1b[32m"), "B")
    assert board.cursor.x == 1
...
def test_write_cell_autowrap():
    ...
    assert board.blitter.current_buffer.get_line_text(1) == "D  "
    assert board.cursor.x == 1
...
def test_clear_rect():
    ...
    for y in range(5):
        for x in range(5):
            board.blitter.current_buffer.set_cell(x, y, "X", "\x1b[31m")
    ...
    for y in range(5):
        for x in range(5):
            if 1 <= x <= 3 and 1 <= y <= 3:
                ...
                assert board.blitter.current_buffer.get_cell(x, y) == (parse_sgr_sequence("\x1b[31m"), "X")
...
def test_clear_screen():
    ...
    for y in range(5):
        for x in range(10):
            board.blitter.current_buffer.set_cell(x, y, chr(ord("A") + y))
    ...
    assert board.blitter.current_buffer.get_line_text(0) == "AAAAAAAAAA"
    assert board.blitter.current_buffer.get_line_text(1) == "BBBBBBBBBB"
    assert board.blitter.current_buffer.get_line_text(2) == "CCCCC     "
    ...
    assert board.blitter.current_buffer.get_line_text(1) == "          "
    assert board.blitter.current_buffer.get_line_text(2) == "      CCCC"
    assert board.blitter.current_buffer.get_line_text(3) == "DDDDDDDDDD"
    assert board.blitter.current_buffer.get_line_text(4) == "EEEEEEEEEE"
    ...
    for y in range(5):
        assert board.blitter.current_buffer.get_line_text(y) == "          "


def test_clear_line():
    board = Board(width=10, height=1)
    for x in range(10):
        board.blitter.current_buffer.set_cell(x, 0, "X")
    ...
    assert board.blitter.current_buffer.get_line_text(0) == "XXXXX     "
    ...
    assert board.blitter.current_buffer.get_line_text(0) == "      XXXX"
...
def test_insert_lines():
    board = Board(width=10, height=5)
    for y in range(5):
        board.blitter.current_buffer.set(0, y, f"Line {y}")
    ...
    assert board.blitter.current_buffer.get_line_text(2) == "          "
    assert board.blitter.current_buffer.get_line_text(3) == "Line 2    "
    ...
    assert board.blitter.current_buffer.get_line_text(1) == "          "
    assert board.blitter.current_buffer.get_line_text(2) == "          "
    assert board.blitter.current_buffer.get_line_text(3) == "Line 1    "
    assert board.blitter.current_buffer.get_line_text(4) == "Line 2    "
...
def test_delete_lines():
    ...
    assert board.blitter.current_buffer.get_line_text(0) == "Line 0    "
    assert board.blitter.current_buffer.get_line_text(1) == "Line 2    "
    ...
    assert board.blitter.current_buffer.get_line_text(0) == "Line 2    "
    assert board.blitter.current_buffer.get_line_text(1) == "Line 3    "
    assert board.blitter.current_buffer.get_line_text(2) == "Line 4    "
    assert board.blitter.current_buffer.get_line_text(3) == "          "
    assert board.blitter.current_buffer.get_line_text(4) == "          "
...
def test_insert_characters():
    ...
    assert board.blitter.current_buffer.get_line_text(0) == "AB   CDEFG"
...
def test_delete_characters():
    ...
    assert board.blitter.current_buffer.get_line_text(0) == "125       "


def test_delete_characters_from_middle_of_line():
    board = Board(width=10, height=5)
    board.blitter.current_buffer.set(0, 0, "123456789")
    board.cursor.x = 2
    board.cursor.y = 0
    board.blitter.delete_characters(3)
    assert board.blitter.current_buffer.get_line_text(0) == "126789    "
...
def test_delete_characters_beyond_end_of_line():
    ...
    assert board.blitter.current_buffer.get_line_text(0) == "12        "
...
def test_delete_last_character_on_line():
    board = Board(width=10, height=5)
    board.blitter.current_buffer.set(0, 0, "abcde")
    board.cursor.x = 4
    board.cursor.y = 0
    board.blitter.delete_characters(1)
    assert board.blitter.current_buffer.get_line_text(0) == "abcd      "
...
def test_insert_characters_at_end_of_line():
    ...
    assert board.blitter.current_buffer.get_line_text(0) == "12345     "
...
def test_insert_characters_with_padding_preserves_style_objects():
    ...
    row = board.blitter.current_buffer.grid[0]
...
def test_delete_characters_preserves_style_objects():
    ...
    board = Board(width=10, height=5)
    board.blitter.current_buffer.set(0, 0, "hello", Style(bold=True))
    board.cursor.x = 1
    board.cursor.y = 0
