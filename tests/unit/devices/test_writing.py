...
def test_write_cell_no_auto_wrap():
    ...
    terminal.board.blitter.write_text("a")
    assert terminal.board.cursor.x == 4
    terminal.board.blitter.write_text("b")
    assert terminal.board.cursor.x == 4
    assert terminal.board.blitter.current_buffer.get_line_text(0) == "    b"
...
def test_write_cell_clip_at_width():
    ...
    assert terminal.board.cursor.x == 4  # Should be clamped to width - 1
    assert terminal.board.blitter.current_buffer.get_line_text(0) == "    X"
...
def test_write_cell_overwrite_at_end_of_line():
    ...
    terminal.board.cursor.y = 0
    terminal.board.blitter.write_text("X")
    assert terminal.board.blitter.current_buffer.get_line_text(0) == "abcX      "
...
def test_write_cell_insert_with_style():
    ...
    terminal.board.cursor.y = 0
    parser.feed("\x1b[31mX")
    assert terminal.board.blitter.current_buffer.get_line_text(0) == "12X345    "
    assert terminal.board.blitter.current_buffer.get_cell(2, 0) == (parse_sgr_sequence("\x1b[31m"), "X")
...
def test_write_cell_insert_at_end_of_line():
    ...
    parser = Parser(terminal.board)
    terminal.board.modes.insert_mode = True
    terminal.board.blitter.current_buffer.set(0, 0, "123")
    terminal.board.cursor.x = 5
    terminal.board.cursor.y = 0
    parser.feed("\x1b[31mX")
    assert terminal.board.blitter.current_buffer.get_line_text(0) == "123  X    "
    assert terminal.board.blitter.current_buffer.get_cell(5, 0) == (parse_sgr_sequence("\x1b[31m"), "X")
...
def test_write_cell_overwrite_at_start_of_line():
    ...
    terminal.board.cursor.y = 0
    parser.feed("\x1b[31mX")
    assert terminal.board.blitter.current_buffer.get_line_text(0) == "X2345     "
    assert terminal.board.blitter.current_buffer.get_cell(0, 0) == (parse_sgr_sequence("\x1b[31m"), "X")
...
def test_write_cell_insert_and_truncate():
    ...
    terminal.board.cursor.y = 0
    parser.feed("\x1b[31mX")
    assert terminal.board.blitter.current_buffer.get_line_text(0) == "12X34"
    assert terminal.board.blitter.current_buffer.get_cell(2, 0) == (parse_sgr_sequence("\x1b[31m"), "X")
...
def test_write_cell_overwrite_middle_of_line_with_style():
    terminal = Board(width=10, height=5)
    terminal.board.blitter.current_buffer.set(0, 0, "0123456789")
    terminal.board.cursor.x = 5
    terminal.board.cursor.y = 0
    terminal.parser.feed("\x1b[31mX")
    assert terminal.board.blitter.current_buffer.get_line_text(0) == "01234X6789"
    assert terminal.board.blitter.current_buffer.get_cell(5, 0) == (parse_sgr_sequence("\x1b[31m"), "X")
...
def test_write_cell_insert_middle_of_line_with_style():
    ...
    terminal.board.cursor.y = 0
    terminal.parser.feed("\x1b[31mX")
    assert terminal.board.blitter.current_buffer.get_line_text(0) == "01234X5678"
    assert terminal.board.blitter.current_buffer.get_cell(5, 0) == (parse_sgr_sequence("\x1b[31m"), "X")
...
def test_write_cell_insert_at_start_of_line_with_style():
    ...
    terminal.board.cursor.y = 0
    terminal.parser.feed("\x1b[31mX")
    assert terminal.board.blitter.current_buffer.get_line_text(0) == "X012345678"
    assert terminal.board.blitter.current_buffer.get_cell(0, 0) == (parse_sgr_sequence("\x1b[31m"), "X")
...
def test_write_cell_insert_at_end_of_line_with_style():
    terminal = Board(width=10, height=5)
    terminal.board.modes.insert_mode = True
    terminal.board.blitter.current_buffer.set(0, 0, "012345678")
    terminal.board.cursor.x = 9
    terminal.board.cursor.y = 0
    terminal.parser.feed("\x1b[31mX")
    assert terminal.board.blitter.current_buffer.get_line_text(0) == "012345678X"
    assert terminal.board.blitter.current_buffer.get_cell(9, 0) == (parse_sgr_sequence("\x1b[31m"), "X")
...
def test_write_cell_overwrite_beyond_end_of_line_with_style():
    ...
    terminal.board.cursor.y = 0
    terminal.parser.feed("\x1b[31mX")
    assert terminal.board.blitter.current_buffer.get_line_text(0) == "abc  X    "
    assert terminal.board.blitter.current_buffer.get_cell(5, 0) == (parse_sgr_sequence("\x1b[31m"), "X")
...
def test_long_print_run_wraps_across_lines():
    ...
    buffer = terminal.board.blitter.current_buffer
