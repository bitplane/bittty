...
def test_set_cursor():
    ...
    terminal.cursor.set_position(100, 30)
    assert terminal.cursor.x == 79  # width - 1
    assert terminal.cursor.y == 23  # height - 1

    terminal.cursor.set_position(-5, -5)
...
def test_line_feed():
    ...
    assert terminal.cursor.x == 10
    assert terminal.cursor.y == 6

    # Test line feed at bottom of terminal (should scroll)
    terminal.cursor.set_position(0, terminal.height - 1)
    terminal.cursor.line_feed()
    assert terminal.cursor.y == terminal.height - 1  # Cursor stays at bottom
...
def test_backspace():
    terminal = Board(width=DEFAULT_TERMINAL_WIDTH, height=DEFAULT_TERMINAL_HEIGHT)
    terminal.cursor.set_position(10, 5)
    terminal.cursor.backspace()
    assert terminal.cursor.x == 9
    assert terminal.cursor.y == 5

    # Test backspace at beginning of line (should wrap)
    terminal.cursor.set_position(0, 5)
    terminal.cursor.backspace()
...
def test_cursor_device_owns_position_and_save_restore():
    ...
    terminal.style.current_ansi_code = "\x1b[32m"
    cursor.restore()
...
def test_cursor_device_line_feed_scrolls_screen_region():
    ...
    terminal.blitter.current_buffer.set(0, 0, "top")
    terminal.blitter.current_buffer.set(0, 1, "mid")
    terminal.blitter.current_buffer.set(0, 2, "bot")

    terminal.cursor.set_position(0, 2)
    terminal.cursor.line_feed()

    assert terminal.cursor.y == 2
    assert terminal.blitter.current_buffer.get_line_text(0).startswith("mid")
    assert terminal.blitter.current_buffer.get_line_text(1).startswith("bot")
