...
def test_scroll_up_within_region():
    ...
    for i in range(10):
        board.blitter.current_page.set(0, i, f"Line {i}")
    ...
    assert board.blitter.current_page.get_line_text(2) == "Line 2    "

    # Lines 3-7 should have scrolled up
    assert board.blitter.current_page.get_line_text(3) == "Line 4    "
    assert board.blitter.current_page.get_line_text(4) == "Line 5    "
    assert board.blitter.current_page.get_line_text(5) == "Line 6    "
    assert board.blitter.current_page.get_line_text(6) == "Line 7    "
    assert board.blitter.current_page.get_line_text(7) == "          "

    # Lines 8-9 should be unchanged
    assert board.blitter.current_page.get_line_text(8) == "Line 8    "
    assert board.blitter.current_page.get_line_text(9) == "Line 9    "
...
def test_scroll_down_within_region():
    ...
    assert board.blitter.current_page.get_line_text(5) == "Line 4    "
    assert board.blitter.current_page.get_line_text(6) == "Line 5    "
    assert board.blitter.current_page.get_line_text(7) == "Line 6    "
...
def test_line_feed_at_bottom_of_scroll_region():
    ...
    assert board.blitter.current_page.get_line_text(0) == "Line 0    "
    assert board.blitter.current_page.get_line_text(1) == "Line 1    "
    assert board.blitter.current_page.get_line_text(6) == "Line 6    "
    assert board.blitter.current_page.get_line_text(7) == "Line 7    "
    assert board.blitter.current_page.get_line_text(8) == "Line 8    "
    assert board.blitter.current_page.get_line_text(9) == "Line 9    "
    ...
    assert board.blitter.current_page.get_line_text(3) == "Line 4    "
    assert board.blitter.current_page.get_line_text(4) == "Line 5    "
    assert board.blitter.current_page.get_line_text(5) == "          "
...
def test_multiple_scroll_regions():
    ...
    assert board.blitter.current_page.get_line_text(0) == "Line 1    "
    assert board.blitter.current_page.get_line_text(4) == "          "

    # Bottom half should be unchanged
    assert board.blitter.current_page.get_line_text(5) == "Line 5    "
    assert board.blitter.current_page.get_line_text(9) == "Line 9    "
    ...
    assert board.blitter.current_page.get_line_text(5) == "Line 6    "
    assert board.blitter.current_page.get_line_text(9) == "          "
...
def test_tmux_style_scroll_only_moves_the_margin_rectangle():
    ...
    assert [page.get_line_text(y) for y in range(6)] == [
        "0000000000",
        "1122222211",
        "2233333322",
        "3344444433",
...
def test_insert_and_delete_lines_honour_left_right_margins():
    ...
    page = board.blitter.current_page
    for y in range(5):
        page.set(0, y, str(y) * 8)

    board.parser.feed("\x1b[?69h\x1b[3;6s\x1b[2;4H\x1b[L")
    assert [page.get_line_text(y) for y in range(5)] == [
        "00000000",
        "11    11",
        "22111122",
        "33222233",
